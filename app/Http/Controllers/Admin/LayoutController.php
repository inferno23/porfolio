...
class LayoutController extends Controller
{
    ...
    public function setHeader(Request $request)
    {
        if ($request->status==NULL) {
            ...
            $request->session()->flash('success','Header Activado');
        }else{
            DB::table('headers')->where('id',$request->id)->update(['is_active' => NULL]);
            $request->session()->flash('error','Header desactivado');
        }
    }
    ...
    public function setAbout(Request $request)
    {
        if ($request->status==NULL) {
            ...
            $request->session()->flash('success','Registro Activado');
        }else{
            DB::table('about')->where('id',$request->id)->update(['is_active' => NULL]);
            $request->session()->flash('error','Registro Desactivado');
        }
    }
    ...
    public function setFooter(Request $request)
    {
        if ($request->status==NULL) {
            ...
            $request->session()->flash('success','Footer Activado');
        }else{
            DB::table('footers')->where('id',$request->id)->update(['is_active' => NULL]);
            $request->session()->flash('error','Footer desactivado');
        }
    }	    
}
