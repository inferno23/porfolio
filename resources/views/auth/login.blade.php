<!DOCTYPE html>
<html lang="en">

<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="keywords" content="Multiespacio Nicasio">
    <meta name="description" content="  Multiespacio Nicasio - Gestión de salon multiuso para eventos sociales, corporativos y culturales.">
    <meta name="author" content="Multiespacio Nicasio">

    <title>Multiespacio Nicasio</title>
    
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/normalize/5.0.0/normalize.min.css">
    <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/animate.css/3.7.0/animate.min.css'>
    <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.1.2/css/bootstrap.min.css'>
    <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css'>

    <link href="{{ asset('templates/backend/sb-admin-2') }}/css/logueo.css" rel="stylesheet">
    
    <link href="https://fonts.googleapis.com/css?family=Poppins:300,400,500,600,700,800,900&display=swap" rel="stylesheet">
    
      <!-- Favicons -->
  <link href="{{ asset('templates/frontend/devfolio') }}/assets/img/favicon.jpeg" rel="icon">
  <link href="{{ asset('templates/frontend/devfolio') }}/assets/img/favicon.jpeg" rel="apple-touch-icon">


</head>

<body>

    <div class="header"><h2></h2></div>
    <div id="wrap">
      <div id="regbar">
        <div id="navthing">
          <h1>Multiespacio <b>Nicasio</b></h1>
        </div>
      </div>
    </div>

<body class="main-bg">
    <div class="login-container text-c animated flipInX">
        <div>
            <h1 class="logo-badge text-whitesmoke" style="color: #002060;"><span class="fa fa-user-circle"></span></h1>
        </div>
        
        <p class="text-whitesmoke" style="color: #002060;">Iniciar Sesión</p>
            <div class="container-content">

                    <div class="card-body p-0">
                        <div class="row">
                            <div class="col-lg">
                                <div class="p-5">
                                    <div class="text-center">
                                        @include('layouts.components.alert-dismissible')
                                    </div>
                                
                                    <form class="margin-t" method="POST" action="{{ route('login.post') }}">
                                        <div class="form-group">
                                            <input type="" class="form-control @error('username') is-invalid @enderror"
                                                id="exampleInputEmail" name="username" aria-describedby="emailHelp"
                                                placeholder="usuario">
                                            @error('username')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                            @enderror
                                        </div>
                                        <div class="form-group">
                                            <input type="password" class="form-control @error('password') is-invalid @enderror"
                                                id="exampleInputPassword" name="password" placeholder="Clave de Acceso">
                                            @error('password')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                            @enderror
                                        </div>
                                        @csrf
                                        <button type="submit" class="form-button button-l margin-b">Ingresar</button>
                                    </form>

                                    <div class="text-center">
                                        <a class="text-darkyellow" href="{{ route('forget.password.get') }}"><small>¿Olvidaste tu Clave de Acceso?</small></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

            </div>
    </div>
    <br>
    <div class="info">
        <h4>multiespacio@impsr.gob.ar - 0341 425-6085 - San Lorenzo 1055, Rosario</h4>
    </div>
    
    <div class="footer">
         <a href="#" alt="Multiespacio NICASIO" target="_blank">
	    	<img src="assets/image/footer.png" style="height: 80px;"> 

         
	    </a>
	</footer>
    <a href="#" class="back-to-top d-flex align-items-center justify-content-center"><i class="bi bi-arrow-up-short"></i></a>
    </div>

    <!-- Bootstrap core JavaScript-->
    <script src="{{ asset('templates/backend/sb-admin-2') }}/vendor/jquery/jquery.min.js"></script>
    <script src="{{ asset('templates/backend/sb-admin-2') }}/vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

    <!-- Core plugin JavaScript-->
    <script src="{{ asset('templates/backend/sb-admin-2') }}/vendor/jquery-easing/jquery.easing.min.js"></script>

    <!-- Custom scripts for all pages-->
    <script src="{{ asset('templates/backend/sb-admin-2') }}/js/sb-admin-2.min.js"></script>
    
    <script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js'></script>


</body>

</html>
