...
<div class="container">
 ...
 <div class="col-xs-6 col-sm-6 col-md-6"> 
      ...
      @foreach($localidades as  $model)
         <option value="{{$model->id}}"  <?php if( $model->id == $localidad_id) echo "selected"; else echo "mm"; ?> > {{$model->localidad}}</option>
         @endforeach
      </select>
    </div>
    </div>
 ...
 <div class="col-xs-6 col-sm-6 col-md-6"> 
      ...
      <option value='TODO' style='background-color: #ffce56;'>Mostrar Datos de toda la  Localidad seleccionada Arriba</option>
    
      @foreach($escuelas as  $model)
         <option value="{{$model->id}}"  <?php if( $model->id == $escuela_id) echo "selected"; else echo "mm"; ?> > {{$model->name}}</option>
         @endforeach
      </select>
    </div>
    </div>
 ...
 <div class="col-xs-6 col-sm-6 col-md-6"> 
      ...
      <select name="mesa_id" id="mesa_id" class="form-control" style='border-color:#1cc88a;'>
      <option value='TODO' style='background-color: #ffce56;'>Mostrar Datos de toda la  Escuela seleccionada Arriba</option>
      @foreach($mesas as  $model)
         <option value="{{$model}}"  <?php if( $model== $mesa_id) echo "selected   style='background-color:#1cc88a;'"; else echo "mm"; ?> > {{$model}}</option>
         @endforeach
...
<?php echo '<br>Reporte de : '.$Nombreprovincia.'<br>total_padron: '.$total_padron;
...
<script>
    ...
    function saveAsPDF() {
        ...
        fecha= 'Graficos_'+provincia+date_format.getDate()+'/'+ (date_format.getMonth()+ 1) +'/'+date_format.getFullYear();
 
       // fecha= 'GraficosMesa '+date_format.getDate()+'/'+ (date_format.getMonth()+ 1) +'/'+date_format.getFullYear();
        var opt = {
            margin: 0.2,
            filename:  fecha,
