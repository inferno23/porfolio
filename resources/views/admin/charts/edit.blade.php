...
<div class="container">
 ...
 <div class="col-xs-6 col-sm-6 col-md-6"> 
      ...
      <select name="escuela_id" id="escuela_id" class="form-control" style='border-color:#1cc88a;'>
      ...
      @foreach($escuelas as  $model)
         <option value="{{$model->id}}"  <?php if( $model->id == $escuela_id) echo "selected  style='background-color:#1cc88a;' "; else echo "mm"; ?> > {{$model->name}}</option>
         @endforeach
      </select>
    </div>
    </div>
...
<script>
    ...
    function saveAsPDF() {
        ...
        fecha= 'GraficosEscuela_'+provincia+date_format.getDate()+'/'+ (date_format.getMonth()+ 1) +'/'+date_format.getFullYear();
        var opt = {
            margin: 0.2,
            filename:  fecha,
