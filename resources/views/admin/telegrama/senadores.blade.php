...
<div class="card shadow mb-4">
    <div class="card-header py-3">
    	<a href="{{ route('telegrama.index') }}" class="btn btn-danger btn-sm">Listado de telegramas</a>
    </div>
    <div class="card-body"><h1>Senadores y Diputados de la provincia: {{ $provincia->provincia }}</h1>
        <form method="POST" action="{{ route('telegrama.storeSenador') }}" enctype="multipart/form-data">
            @csrf
                           ...
                           @endif
                            ...
                            <td  width="100%" colspan="3" >   <label>{{ $model->partido }}</label>
                             ...
                             <td>
                                ...
                                <input class="form-control" style="width: 80px;" onchange="suma(this.form);" name="valor6[]" id="valor6" type="number" min="0"> 
                                @else
                                ...
                                <input class="form-control" style="width: 80px;" onchange="sumadiputado(this.form);" name="valor7[]" id="valor7" type="number" min="0"> 
                                @else
...
<tr>
<td colspan="4">
                           <label class="text-success">Total de Votos. Datos  de Verificacion </label>
                </td>
                           <td>
                        
                           <input class="form-control" style=" border-color: #1DF591;" type="text" name="total" id="total" value="0" />
                           </td>

                            <td ><input class="form-control"  style=" border-color: #1DF591;" type="text" name="totaldiputado" id="totaldiputado" value="0" /> 
                           </td>
...
suma=function(f){
    ...
    for(var x=0;x<f.length;x++){//recorremos los campos dentro del form
        if(f[x].name.indexOf('valor6')!=-1){//si el nombre campo contiene la palabra 'aporte'
...
sumadiputado=function(f){
    var total=0;
    for(var x=0;x<f.length;x++){//recorremos los campos dentro del form
        if(f[x].name.indexOf('valor7')!=-1){//si el nombre campo contiene la palabra 'aporte'
            total+=Number(f[x].value);//sumamos, convirtiendo el contenido del campo a número
        }
    }
    document.getElementById('totaldiputado').value=total;//al final colocamos la suma en algún input.
}
